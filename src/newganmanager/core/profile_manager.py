...
class Profile_Manager(Config_Manager):
    ...
    def create_profile(self, name):
        ...
        self.save_config(self.root_dir+"/.user/"+name+".json", {"imgs": {}, "ethnics": {}, "img_dir": "", "rtf": ""})
        try:
    ...
    def swap_xml(self, deact_name, act_name, deact_img_dir, act_img_dir):
        ...
        if os.path.isfile(deact_config_path):
            try:
                with open(deact_user_path, 'wb') as output, open(deact_config_path, 'rb') as input:
                    copyfileobj(input, output)
            except (OSError, IOError) as e:
                raise Exception(f"Failed to save deactivated profile config from {deact_config_path} to {deact_user_path}: {str(e)}")
        ...
        if os.path.isfile(act_config_path):
            try:
                with open(act_config_path, 'wb') as output, open(act_user_path, 'rb') as input:
                    copyfileobj(input, output)
            except (OSError, IOError) as e:
                raise Exception(f"Failed to restore activated profile config from {act_user_path} to {act_config_path}: {str(e)}")
