...
class Test_RTF_Parser(unittest.TestCase):
    ...
    def test_parse_rtf_with_short_UIDs(self):
        test_simple_UID = RTF_Parser.parse_rtf(None, "testing/test_simple_UID.rtf")
    ...
    def test_parse_rtf_fake_players(self):
        test_simple_UID_fake = RTF_Parser.parse_rtf(None, "testing/test_simple_UID_fake.rtf")
    ...
    def test_valid_rtf(self):
        self.assertTrue(RTF_Parser.is_rtf_valid(None, "testing/test_simple_UID.rtf"))
        self.assertFalse(RTF_Parser.is_rtf_valid(None, "testing/false.rtf"))


class Test_Config_Manager(unittest.TestCase):

    def test_get_latest_prf(self):
        latest_prf = Config_Manager().get_latest_prf("testing/simple_cfg.json")
...
class Test_Profile_Manager(unittest.TestCase):
    ...
    def test_delete_profile(self):
        cfg = Config_Manager().load_config("testing/.user/eth_cfg.json")
        cfg["Profile"] = {"testmig" : False, "No Profile": True}
        Config_Manager().save_config("testing/.user/cfg.json", cfg)
        f = open("testing/.user/testmig.xml", "a")
        f.write("TESTMIGXML!")
        f.close()
        f = open("testing/.user/testmig.json", "a")
        f.write("{'text': 'TESTMIGJSON!'}")
        f.close()
        self.pm.config = Config_Manager().load_config("testing/.user/cfg.json")
        self.pm.delete_profile("testmig")
        usr_cfg = Config_Manager().load_config("testing/.user/cfg.json")
        self.assertNotIn("testmig", usr_cfg["Profile"])
        self.assertIn("No Profile", usr_cfg["Profile"])
        self.assertFalse(os.path.isfile("testing/.user/testmig.json"))
        self.assertFalse(os.path.isfile("testing/.user/testmig.xml"))

    def test_create_profile(self):
        self.pm.create_profile("test")
        cfg = Config_Manager().load_config("testing/.user/cfg.json")
        self.assertFalse(cfg["Profile"]["test"])
        self.assertTrue(os.path.isfile("testing/.user/test.json"))
        self.assertTrue(os.path.isfile("testing/.user/test.xml"))
        prf_cfg = Config_Manager().load_config("testing/.user/test.json")
    ...
    def test_load_profile(self):
        ...
        with open("../testing/config.xml", "r") as cfg_xml:
            data = cfg_xml.read()
    ...
    def test_swap_xml(self):
        ...
        with open("testing/.user/test.xml", "r") as test_xml:
            self.assertEqual(test_xml.read(), "OUTSIDE")
        with open("../testing/config.xml", "r") as config_xml:
    ...
    def test_migrate_function(self):
        ...
        res_cfg = Config_Manager().load_config("testing/.config/eth_cfg.json")
        ...
        self.assertTrue(os.path.isfile("testing/.user/testmig.json"))
        self.assertTrue(os.path.isfile("testing/.user/testmig.xml"))
